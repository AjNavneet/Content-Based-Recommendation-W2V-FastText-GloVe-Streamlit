...
# Function to drop null values from a dataframe
def drop_null(df):
    try:
...
# Function to drop duplicates from a dataframe's column    
...
def convert_lower(df, col_to_convert, new_col):
    try:
        df[new_col] = df[col_to_convert].apply(lambda x: x.lower())  
    except Exception as e:
        print(e)
    else:
        return df
...
def process_data(df, col1, col2):
    ...
    # Dropping Duplicates and keeping the first record
    ...
    # Converting Description to a List
